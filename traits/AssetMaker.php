...
trait AssetMaker
{
    ...
    public function makeAssets($type = null)
    {
        ...
        if ($type == null || $type == 'preload:js') {
            foreach ($this->assets['js'] as $asset) {
                if ($attributes = $this->renderAssetAttributes('css', ['path'=>$asset['path'],'attributes'=>['rel'=>'preload','type'=>'script']])) {
                    $result .= "<link {$attributes} />" . PHP_EOL;
                }
            }

 ...
 */
class SplitFormController extends ControllerBehavior
{
     ...
     */
    protected function validateField($type)
    {
        $recordId = $this->recordId ?? post('id');
        ...
        try {
            switch ($type) {
                case 'create':
                    ...
                    $model = $this->formExtendModel($model) ?: $model;# code...
                    break;
                case 'update':
                    $this->controller->vars['recordId'] = $recordId;
    ...
    public function onClickSplitLayoutRecord()
    {
        return $this->onSplitManageForm('update');
    }

    public function onClickSplitAddButton()
    {
        return $this->onSplitManageForm('create');
    }

    protected function beforeSplitAjax($type)
    {
        if ($this->initialized) {
            return;
        }
     ...
     */
    public function splitMakePartial($partial, $params = [])
    {
        $contents = $this->makePartial('split_'.$partial, $params + $this->vars, false);
        if (!$contents) {
    ...
    public function onSplitManageForm($view)
    {
        $this->beforeSplitAjax($view);
        ...
        // return $this->splitMakePartial($view);
    }
     ...
     */
    public function splitSave($recordId = null, $context = null)
    {
        if(!empty($recordId)) {
            $type = 'update';
            $_context = self::CONTEXT_UPDATE;
        }else{
            $type = 'create';
            $_context = self::CONTEXT_CREATE;
        }
        $this->context = strlen($context) ? $context : $this->getConfig($type."[context]", $_context);
        ...
        $this->{'formBefore'.ucfirst($type)}($model);
        ...
        $this->{'formAfter'.ucfirst($type)}($model);

        Flash::success($this->getLang("{$this->context}[flashSave]", 'backend::lang.form.'.$type.'_success'));

        $refreshList = $this->controller->listRefresh('manage');

        if($type == 'create') {

        }
        if ($reloadSplitRight = $this->reloadSplitRight($this->context, $model)) {
            $refreshList = array_merge((array)$reloadSplitRight,$refreshList);
        }
    ...
    public function reloadSplitRight($context = null, $model = null)
    {
        if(post('create')){
            return $this->onSplitManageForm('create');
        }else if ($model && !post('create')) {
    ...
    public function onSplitSave($recordId = null, $context = null) {
        $recordId = post('record.id');
        return $this->splitSave($recordId, $context);
    }

    public function onSplitCreate($context = null) {
        return $this->splitSave(null, $context);
    }
     ...
     */
    public function onSplitDelete($recordId = null)
    {
        $recordId = post('record.id');
        $this->context = $this->getConfig('update[context]', self::CONTEXT_UPDATE);
        $model = $this->formFindModelObject($recordId);
        ...
        return $this->controller->listRefresh('manage');
    }
     ...
     * @return void
     */
    public function onSplitPreview($recordId = null, $context = null)
    {
        $recordId = post('id');
        try {
            $this->context = strlen($context) ? $context : $this->getConfig('preview[context]', self::CONTEXT_PREVIEW);
            $this->pageTitle = $this->pageTitle ?: $this->getLang(
                "{$this->context}[title]",

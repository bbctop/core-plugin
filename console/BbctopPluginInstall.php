 ...
 */
class BbctopPluginInstall extends Command
{
    /**
     * @var string name of console command
     */
    protected $name = 'bbctop-plugin:install';

    /**
     * @var string description of the console command
     */
    protected $description = 'Install a plugin from the Bbctop marketplace & October marketplace.';
    ...
    public function handle()
    {
        ...
        if (strpos($name,'bbctop.') === 0) {
            $this->output->writeln("<info>Plugin from BBCTOP</info>");
            $composerCode = System::octoberToComposerCode(
                $name,
                'plugin',
                (bool) $this->option('oc')
            );
            $this->addBbctopComposer();
     ...
     * addRepoFromSource adds a plugin to composer's repositories
     */
    protected function addBbctopComposer()
    {
        $composer = new ComposerProcess;
        $composer->addRepository('bbctop', 'composer', 'https://gateway.f.bbctop.cn/packages');
    }

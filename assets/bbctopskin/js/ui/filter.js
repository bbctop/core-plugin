...
+function ($) { "use strict";
                    ...
                    <div id="controlFilterPopover" class="control-filter-popover control-filter-box-popover is-range">  \
                        <div class="filter-search loading-indicator-container size-input-text">                         \
                            ...
                            <div class="filter-items">                                                                  \
                                <ul>                                                                                    \
                                    {{#available}}                                                                      \
                                        <li data-item-id="{{id}}" data-token="{{name}}"><a href="javascript:;">{{name}}</a></li>              \
                                    {{/available}}                                                                      \
                                    {{#loading}}                                                                        \
    ...
    FilterWidget.prototype.init = function() {
        ...
        this.$el.on('show.oc.popover', 'a.filter-scope', function(event){
            ...
            $(event.relatedTarget).on('input', '#controlFilterPopover [data-track-search]', function(){
    ...
    // Search items with js only
    FilterWidget.prototype.searchItem = function($input) {
        var value = $input.val(),
            $parent = $input.parent();
            $parent.find('.filter-items [data-token].show').removeClass('show')
        console.log(value)
        if(value) {
            $parent.addClass('searching');
            $parent.find('.filter-items [data-token*="'+value+'"]').addClass('show')
        }else{
    ...
    FilterWidget.prototype.displayPopover = function($scope) {
        var self = this,
            scopeName = $scope.data('scope-name'),
            noajaxsearch = $scope.data('scope-no-ajax-search'),
            data = null,
            isLoaded = true,
            container = false;
        ...
        data.searchSetting = '\
        data-request="'+self.options.optionsHandler+'"                                          \
        data-load-indicator-opaque                                                              \
        data-load-indicator                                                                     \
        data-track-input ';
        
        // check if noajaxsearch is true just using js search only
        if(noajaxsearch == true) data.searchSetting = 'data-track-search'
    ...
    FilterWidget.prototype.addItemsToListElement = function($ul, items) {
        $.each(items, function(key, obj) {
            var item = $('<li />').data({ 'item-id': obj.id }).attr('data-token',obj.name)
